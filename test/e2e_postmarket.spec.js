...
describe("End-End Test (Post-Market)", function () {
	//If not in Post-Market, skip the test
	...
	if (
		currentTimeString >= "16:00:00" &&
		currentTimeString <= "19:59:00" &&
		day != 6 &&
		day != 0
	) {
		var stockSocket;
		...
		it("marketHours property: is equal to POST_MARKET", function (done) {
			expect(wsData.marketHours === "POST_MARKET");
			done();
		});
		...
		console.log("Can't run post-market e2e test outside of post-market hours");
	}
});
