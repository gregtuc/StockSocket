...
describe("End-End Test (Regular-Market)", function () {
	//If not in Regular-Market, skip the test
	...
	if (
		currentTimeString >= "09:30:00" &&
		currentTimeString <= "15:59:00" &&
		day != 6 &&
		day != 0
	) {
		var stockSocket;
		...
		it("Data Object is of type object", function (done) {
			expect(wsData).to.be.an("object");
			done();
		});

		it("Data Object is greater than length 1", function (done) {
		...
		it("marketHours property: is equal to REGULAR_MARKET", function (done) {
		...
		//DayVolume Property
		it("dayVolume property: exists", function (done) {
			expect(wsData.hasOwnProperty("dayVolume"));
			done();
		});
		it("dayVolume property: is type string", function (done) {
			expect(typeof wsData.dayVolume === "string");
			done();
		});
		it("dayVolume property: is not null", function (done) {
